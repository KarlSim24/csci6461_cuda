...
// Matrix multiplication kernel called by MatMul()
 __global__ void ShMemMatMulKernel(Matrix A, Matrix B, Matrix C)
{
...
void MatMul(const Matrix A, const Matrix B, Matrix C)
{
    ...
    ShMemMatMulKernel<<<dimGrid, dimBlock>>>(d_A, d_B, d_C);
...
int main() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        Matrix A = {N, N, N, new float[N * N]};
        Matrix B = {N, N, N, new float[N * N]};
        Matrix C = {N, N, N, new float[N * N]};
